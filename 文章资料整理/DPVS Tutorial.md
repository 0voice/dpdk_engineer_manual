...
- Terminology
  - [*One-arm* and *two-arm*](https://toscode.gitee.com/zc520yzy/dpvs/blob/master/doc/tutorial.md#one-two-arm)
  - [*KNI* Device](https://toscode.gitee.com/zc520yzy/dpvs/blob/master/doc/tutorial.md#kni)
- Full-NAT Mode
  - Simple Full-NAT (two-arm)
    - [Something about *LIP*, *routes* and *TOA*](https://toscode.gitee.com/zc520yzy/dpvs/blob/master/doc/tutorial.md#lip-routes-toa)
  - [Full-NAT with OSPF/ECMP (two-arm)](https://toscode.gitee.com/zc520yzy/dpvs/blob/master/doc/tutorial.md#fnat-ospf)
  - [Full-NAT with Keepalived (one-arm)](https://toscode.gitee.com/zc520yzy/dpvs/blob/master/doc/tutorial.md#fnat-keepalive)
- [DR Mode (one-arm)](https://toscode.gitee.com/zc520yzy/dpvs/blob/master/doc/tutorial.md#dr)
- [Tunnel Mode(one-arm)](https://toscode.gitee.com/zc520yzy/dpvs/blob/master/doc/tutorial.md#tunnel)
- [NAT Mode(one-arm)](https://toscode.gitee.com/zc520yzy/dpvs/blob/master/doc/tutorial.md#nat)
- [SNAT Mode (two-arm)](https://toscode.gitee.com/zc520yzy/dpvs/blob/master/doc/tutorial.md#snat)
- Virtual devices
  - [Bonding Device](https://toscode.gitee.com/zc520yzy/dpvs/blob/master/doc/tutorial.md#vdev-bond)
  - [VLAN Device](https://toscode.gitee.com/zc520yzy/dpvs/blob/master/doc/tutorial.md#vdev-vlan)
  - [Tunnel Device](https://toscode.gitee.com/zc520yzy/dpvs/blob/master/doc/tutorial.md#vdev-tun)
  - [KNI for virtual device](https://toscode.gitee.com/zc520yzy/dpvs/blob/master/doc/tutorial.md#vdev-kni)
- [UDP Option of Address (UOA)](https://toscode.gitee.com/zc520yzy/dpvs/blob/master/doc/tutorial.md#uoa)
- [Launch DPVS in Virtual Machine (Ubuntu)](https://toscode.gitee.com/zc520yzy/dpvs/blob/master/doc/tutorial.md#Ubuntu16.04)

> To compile and launch DPVS, pls check *README.md* for this project.

## 1. Terminology

About the concepts of *Full-NAT* (`FNAT`), `DR`, `Tunnel`, `toa`, `OSPF`/`ECMP` and `keepalived`, pls refer [LVS](https://toscode.gitee.com/zc520yzy/dpvs/blob/master/doc/www.linuxvirtualserver.org) and [Alibaba/LVS](https://github.com/alibaba/LVS/tree/master/docs).
...
![kni](https://toscode.gitee.com/zc520yzy/dpvs/raw/master/doc/pics/kni.png)
...
![fnat-two-arm](https://toscode.gitee.com/zc520yzy/dpvs/raw/master/doc/pics/fnat-two-arm.png)
...
#### 2.1.1 Something about *LIP*, *routes* and *TOA*
...
### 2.2 Full-NAT with OSPF/ECMP (two-arm)
...
![fnat-ospf-two-arm](https://toscode.gitee.com/zc520yzy/dpvs/raw/master/doc/pics/fnat-ospf-two-arm.png)
...
![fnat-ospfd-kni.png](https://toscode.gitee.com/zc520yzy/dpvs/raw/master/doc/pics/fnat-ospfd-kni.png)

Now the configuration has two parts, one is for `dpvs` and another is for `zebra/ospfd`.

`dpvs` part is almost the same with the example in [simple fnat](https://toscode.gitee.com/zc520yzy/dpvs/blob/master/doc/tutorial.md#simple-fnat), except
...
### 2.3 Full-NAT with Keepalived (one-arm)
...
![fnat-keepalive](https://toscode.gitee.com/zc520yzy/dpvs/raw/master/doc/pics/fnat-keepalived.png)
...
![dr-one-arm](https://toscode.gitee.com/zc520yzy/dpvs/raw/master/doc/pics/dr-one-arm.png)
...
> `DPVS` needs a *RS-faced* IP itself (here means "LAN-side" IP, it's not the same conception as Local-IP (LIP) used by FNAT, just a normal IP address). Because `DPVS` need communicated with `RS`es. For *one-arm*, this LAN IP and VIP are on same DPDK interface. But they cannot be same, because `VIP` will also be set on `RS`es, if we do not use a separated LAN-IP, `RS`es will not reply the ARP request. Furthermore, the LAN-IP of `DPVS` must be added before VIP. For *tow-arm* DR, `DPVS` also need a LAN side IP to talk with LAN-side hosts, while VIP is configured on client-faced (WAN) interface.
...
> DR mode for two-arm is similar with [two-arm FNAT](https://toscode.gitee.com/zc520yzy/dpvs/blob/master/doc/tutorial.md#simple-fnat), pls change the forwarding mode by `ipvsadm -g`, and you need NOT config `LIP`. Configuration of `RS`es are the same with one-arm.

## 4. Tunnel Mode (one-arm)
...
![tunnel-one-arm](https://toscode.gitee.com/zc520yzy/dpvs/raw/master/doc/pics/tunnel-one-arm.png)
...
![nat-one-arm](https://toscode.gitee.com/zc520yzy/dpvs/raw/master/doc/pics/nat-one-arm.png)
...
![bond-vlan-kni](https://toscode.gitee.com/zc520yzy/dpvs/raw/master/doc/pics/bond-vlan-kni.png)
...
The example C code for RS to fetch Real Client IP can be found [here](https://toscode.gitee.com/zc520yzy/dpvs/blob/master/uoa/example/udp_serv.c).
...
Actually, we use private IP option to implement `UOA`, pls check the details in [uoa.md](https://toscode.gitee.com/zc520yzy/dpvs/blob/master/uoa/uoa.md).

## 9. Launch DPVS in Virtual Machine (Ubuntu)

### 9.1 DPDK build and install
...
转载自：  作者 [tycoon3](https://home.cnblogs.com/u/dream397/)    本文链接 https://www.cnblogs.com/dream397/p/14766014.html